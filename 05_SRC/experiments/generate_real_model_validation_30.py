"""
generate_real_model_validation_30.py

Generates real model outputs for the validation_30 prompt set via OpenRouter.
Outputs a CSV with the same schema expected by run_judge_calibration.py --input-csv,
so the result can be scored directly without any reformatting.

Input  : 04_RUNS/real_model_validation_30/real_model_validation_30_prompts.csv
Output : 04_RUNS/real_model_validation_30/real_model_validation_30_outputs.csv
...
_PROMPTS_CSV = (
    ...
    / "real_model_validation_30_prompts.csv"
)
_OUTPUT_CSV  = (
    _REPO_ROOT / "04_RUNS" / "real_model_validation_30"
    / "real_model_validation_30_outputs.csv"
)
...
def main(
    ...
    if dry_run:
        ...
        print()
        print("DRY-RUN — generation plan (no API calls, no file writes)")
        print()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate real model outputs for the validation_30 prompt set via OpenRouter."
    )
    parser.add_argument(
        "--model",
